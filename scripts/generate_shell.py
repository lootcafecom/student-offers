...
function rowHtml(o, idx) {
  ...
  return `
    <a class="offer-row" style="--glow-rgb: ${rgb};" href="${o.url}" target="_blank" rel="noopener">
      ${rankHtml}
      <div class="favicon">${iconHtml}</div>
      <div class="row-main">
        <span class="row-name">${escapeHtml(o.name)}
          ${o.pack_verified ? '<span class="verified-badge">GITHUB ED. PARTNER</span>' : ''}
...
function renderCategorySections() {
  ...
  container.innerHTML = CATEGORIES.map(name => {
    ...
    const preview = inCat.slice(0, 8);
    return `
      <section class="cat-section">
        <div class="cat-section-head">
          <h2><span class="dot" style="background:${color}"></span>${name}</h2>
          <a class="see-all" href="/categories/${slugify(name)}/">See all ${COUNTS[name]} &rarr;</a>
        </div>
        <div class="row-list">${preview.map((o, i) => rowHtml(o, i)).join('')}</div>
      </section>
    `;
  }).join('');
}
